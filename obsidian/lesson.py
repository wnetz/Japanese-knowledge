    ...
    def _parse_lessons(self, body: str) -> list[Lesson]:
        ...
        for section in parse_sections(body):
            ...
            if level == 2 and heading.casefold().replace(" ", "") == "cando":
                current.can_do.extend(item.value for item in items)
    ...
    def _add_practice_items(practice: Practice, items: list[ContentItem]) -> None:
        for item in items:
            if item.type == "question":
                practice.questions.append(item.value)
            elif item.type == "answer":
                practice.responses.append(item.value)
            elif item.type == "pattern":
                practice.patterns.append(item.value)
            else:
                practice.skills.append(item.value)
